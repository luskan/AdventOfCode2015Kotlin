...
fun runday19() {
    ...
    println(" --- Day19 ---")
    println("result for part 1: ${verifyResult(calculateNumberOfMolecules(data), 509)}") // 509

    // Way too timeconsuming
    //println("result for part 2: ${calculateNumberOfStepsBruteForce(data)}") // 195

    // This one works but it does not go all the possibilities which renders it useless for some inputs - I suppose.
    //println("result for part 2: ${verifyResult(calculateNumberOfStepsTopDown(data), 195)}") // 195
    println("result for part 2: ${verifyResult(calculateNumberOfStepsTopDownSeq(data), 195)}") // 195
}
